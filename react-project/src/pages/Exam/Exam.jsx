        ...
        <>
            ...
            <header className="fixed top-0 left-0 right-0 z-50 bg-surface border-b border-outline-variant shadow-sm transition-all duration-200">
                <div className="flex justify-between items-center w-full px-lg py-md max-w-container-max mx-auto">
                    <div className="flex items-center gap-md">
                        <Link className="font-headline-md text-headline-md font-bold text-pink-600" to="/">Pan Học Code 🌸</Link>
                        <div className="h-6 w-px bg-pink-100 hidden md:block mx-sm"></div>
                        <div className="hidden md:flex flex-col">
                            <span className="font-label-md text-label-md text-pink-700">{subject}</span>
            ...
            <main className="flex-grow pt-xxl pb-xxl px-md mt-md">
                <div className="max-w-3xl mx-auto mt-lg">
                    <div className="card-sakura p-xl mb-lg animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex justify-between items-start mb-lg">
                            <span className="bg-pink-50 text-pink-600 px-md py-xs rounded-full font-label-md text-label-md border border-pink-100">
                                Multiple Choice 🌸
                            </span>
                            <button className="text-secondary hover:text-pink-600 transition-colors flex items-center gap-xs">
                                <span className="material-symbols-outlined text-[20px]">flag</span>
                                <span className="font-label-md text-label-md">Flag for review</span>
                            </button>
                        </div>
                        <h2 className="font-headline-sm text-headline-sm text-pink-900 mb-xl leading-relaxed">
                        ...
                        <div className="space-y-md" id="options-container">
                            {currentQuestion.options.map((option, idx) => (
                                <label 
                                    ...
                                    className={`option-card relative flex items-center p-lg border rounded-xl cursor-pointer transition-all active:scale-[0.99] group ${answers[currentQuestion.id] === idx ? 'border-pink-500 bg-pink-50/40 ring-1 ring-pink-500' : 'border-pink-100'}`}
                                >
                                    <input className="hidden" name="exam-option" type="radio" checked={answers[currentQuestion.id] === idx} readOnly />
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-lg transition-colors ${answers[currentQuestion.id] === idx ? 'border-pink-500' : 'border-pink-200 group-hover:border-pink-400'}`}>
                                        <div className={`w-3 h-3 rounded-full bg-pink-500 transition-opacity ${answers[currentQuestion.id] === idx ? 'opacity-100' : 'opacity-0'}`}></div>
                                    </div>
            ...
            <footer className="fixed bottom-0 left-0 right-0 z-50 bg-surface border-t border-pink-100">
                <div className="w-full py-md px-lg flex justify-between items-center max-w-container-max mx-auto">
                    <button 
                        onClick={handlePrev}
                        disabled={currentIndex === 0}
                        className={`flex items-center gap-sm px-lg py-md border border-pink-200 rounded-full text-pink-600 font-label-md text-label-md transition-all ${currentIndex === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-pink-50 active:opacity-80'}`}
                    >
                        <span className="material-symbols-outlined">chevron_left</span> Previous
                    </button>
                    
                    <div className="flex items-center gap-md">
                        <button className="hidden sm:flex items-center gap-sm px-lg py-md text-pink-600 font-label-md text-label-md hover:underline decoration-pink-500 underline-offset-4">
                            Jump to Question...
                        </button>
                        <button 
                            onClick={currentIndex === questions.length - 1 ? () => setShowModal(true) : handleNext}
                            className="flex items-center gap-sm btn-sakura px-lg py-md rounded-full"
                        >
                            {currentIndex === questions.length - 1 ? 'Submit Exam' : 'Next'} <span className="material-symbols-outlined">{currentIndex === questions.length - 1 ? 'check' : 'chevron_right'}</span>
                        </button>
                    </div>
            ...
            {showModal && (
                <div className="fixed inset-0 z-[100] bg-on-surface/40 backdrop-blur-sm flex items-center justify-center p-md" id="finish-modal">
                    <div className="bg-surface p-xl rounded-xl shadow-xl max-w-md w-full border-2 border-pink-200 animate-in zoom-in-95 duration-200">
                        <h3 className="font-headline-sm text-headline-sm text-pink-700 mb-md">Finish Examination? 🌸</h3>
                        ...
                        <div className="flex flex-col sm:flex-row gap-md">
                            <button onClick={() => setShowModal(false)} className="flex-1 py-md border border-pink-200 rounded-full font-label-md text-label-md text-pink-600 hover:bg-pink-50 transition-colors" >
                                Go Back
                            </button>
                            <button onClick={handleSubmit} className="flex-1 py-md btn-sakura rounded-full font-label-md text-label-md transition-colors inline-block text-center flex items-center justify-center">
                                Submit Exam
                            </button>
                        </div>
                    </div>
